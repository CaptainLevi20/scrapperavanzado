...
1. Se lee el archivo de origen completo a memoria **antes** de tocar el destino: `buffer = await (await entry.fileHandle.getFile()).arrayBuffer()`. (`File.stream()` no está soportado por el entorno de test — jsdom — así que se usa `arrayBuffer()`, que sí lo está; esto ya resuelve el problema real, porque el límite de memoria pasa a ser **un archivo a la vez**, no el lote completo — para documentos individuales de tamaño normal, mucho más chico que el lote entero, esto es holgado.)
2. `yearDirHandle = entry.yearFolder ? await outputRoot.getDirectoryHandle(entry.yearFolder, { create: true }) : outputRoot` — recrea la carpeta de año en el destino (o escribe directo en la raíz si el archivo no tenía carpeta de año, igual que el guard `entry.yearFolder ? ... : finalName` que ya existe en `build.ts`). Se hace después de leer el origen a propósito: `getFileHandle(..., { create: true })` crea el archivo de destino de inmediato (incluso vacío), así que si se hiciera antes y la lectura del origen fallara, quedaría un archivo vacío huérfano en la carpeta de salida.
3. `outFileHandle = await yearDirHandle.getFileHandle(finalName, { create: true })`; `writable = await outFileHandle.createWritable()`; `await writable.write(buffer)` seguido de `await writable.close()`.
