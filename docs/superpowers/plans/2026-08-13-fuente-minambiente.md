...
- [x] `pytest tests/families/test_minambiente.py -v` — 33 tests, all PASS.
- [x] `pytest` (full suite) — 777 passed, 1 pre-existing failure unrelated to this change (`test_alembic_upgrade_head_creates_all_tables`, a `WinError 2` from a missing executable in this machine's PATH — confirmed to fail the same way in isolation, before this branch existed). `tests/test_seed.py`'s hardcoded family/source counts were bumped by one family + one source (`core/seed.py`'s `_FAMILIES` dict and both count assertions updated together).
- [ ] Manual sanity check against the live site (not mocked): run `ScrapMinAmbiente().scrap(fini=..., ffin=..., limit=5, on_progress=print)` and confirm real documents come back with clean, non-`title_unverified` titles in the expected shape, including at least one Circular with a `C_MADS_{código}_{año}` title.
