...
def test_seed_running_concurrently_does_not_crash_or_duplicate_rows(test_engine, db_session):
    ...
    try:
        ...
        assert len(sources) == 1 + 28 + 21 + 33 + 6
    finally:
...
def test_seed_populates_families_and_sources_and_is_idempotent(db_session):
    ...
    # 1 (Corte Constitucional) + 28 (SAMAI) + 21 (fuente única: corte_suprema, jep, cndj,
    # adr, adres, ane, anh, mincit, madr, minambiente, minvivienda, mineducacion,
    # mininterior, mindeporte, minjusticia, minenergia, mintrabajo, superfinanciera, supersalud, ssf, snr) + 33 (Tribunales Superiores, incl. Bogotá D.C.) + 6 (tipos de Juzgado) = 89
    assert len(sources) == 1 + 28 + 21 + 33 + 6
