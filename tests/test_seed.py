...
def test_seed_running_concurrently_does_not_crash_or_duplicate_rows(test_engine, db_session):
    ...
    try:
        ...
        assert len(sources) == 1 + 28 + 11 + 33 + 6
    finally:
...
def test_seed_populates_families_and_sources_and_is_idempotent(db_session):
    ...
    assert {f.key for f in families} == {
        ...
        "adr", "adres", "ane", "anh", "rama_judicial", "mincit", "madr", "minambiente", "minvivienda",
    }

    sources = repository.list_sources(db_session)
    # 1 (Corte Constitucional) + 28 (SAMAI) + 11 (fuente única: corte_suprema, jep, cndj,
    # adr, adres, ane, anh, mincit, madr, minambiente, minvivienda) + 33 (Tribunales
    # Superiores, incl. Bogotá D.C.) + 6 (tipos de Juzgado) = 79
    assert len(sources) == 1 + 28 + 11 + 33 + 6
