...
def test_seed_populates_families_and_sources_and_is_idempotent(db_session):
    ...
    assert {f.key for f in families} == {
        ...
        "adr", "adres", "ane", "anh", "rama_judicial", "mincit", "madr", "minambiente",
    }

    sources = repository.list_sources(db_session)
    # 1 (Corte Constitucional) + 28 (SAMAI) + 10 (fuente única: corte_suprema, jep, cndj,
    # adr, adres, ane, anh, mincit, madr, minambiente) + 33 (Tribunales Superiores, incl. Bogotá D.C.) +
    # 6 (tipos de Juzgado) = 78
    assert len(sources) == 1 + 28 + 10 + 33 + 6
