...
def test_seed_running_concurrently_does_not_crash_or_duplicate_rows(test_engine, db_session):
    ...
    try:
        ...
        assert len(sources) == 1 + 28 + 12 + 33 + 6
    finally:
...
def test_seed_populates_families_and_sources_and_is_idempotent(db_session):
    ...
    assert {f.key for f in families} == {
        ...
        "minambiente", "minvivienda", "mineducacion",
    }

    sources = repository.list_sources(db_session)
    # 1 (Corte Constitucional) + 28 (SAMAI) + 12 (fuente única: corte_suprema, jep, cndj,
    # adr, adres, ane, anh, mincit, madr, minambiente, minvivienda, mineducacion) + 33
    # (Tribunales Superiores, incl. Bogotá D.C.) + 6 (tipos de Juzgado) = 80
    assert len(sources) == 1 + 28 + 12 + 33 + 6
